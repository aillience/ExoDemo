...
import com.huoli.bbl.exodemo.glide.GlideThumbnailTransformation;
...
public class ExoPlayerManager implements PreviewLoader, PreviewBar.OnScrubListener {
    ...
    public void play(Uri uri) {
        Log.i("exo","设置播放:"+uri.getPath());
        mediaSourceBuilder.setUri(uri);
    }

    public void onStart() {
        if (Util.SDK_INT > 23) {
            Log.i("exo onStart","开始播放:"+ Util.SDK_INT);
            createPlayers();
        }
    }
    ...
    public void onResume() {
        if (Util.SDK_INT <= 23) {
            Log.i("exo onResume","开始播放:"+ Util.SDK_INT);
            createPlayers();
        }
    }
